...
import com.horadoshow.robot.trading.broker.AccountState;
import com.horadoshow.robot.trading.broker.Position;
import com.horadoshow.robot.trading.broker.SimulatedBroker;
import java.math.BigDecimal;
import java.util.List;
import java.util.Map;

/**
 * Executes backtests for a single symbol using historical candles provided by a {@link MarketDataClient}.
 * <p>
 * This engine is responsible for driving the clock over historical data, calling the strategy on each bar,
 * and applying the resulting trades to a {@link SimulatedBroker}.
 */
  ...
  public BacktestResult runSingleSymbol(
    ...
    for (Candle c : daily) {
      ...
      if (decision.type() == DecisionType.BUY) {
        BigDecimal toInvest = state.cash().multiply(
            BigDecimal.valueOf(decision.positionSizePct() == null ? 1.0 : decision.positionSizePct())
        );
        ...
        BigDecimal qty = position.quantity().multiply(
            BigDecimal.valueOf(decision.positionSizePct() == null ? 1.0 : decision.positionSizePct())
        );
