 ...
 *
 * <p>This class focuses on simple, commonly used indicators for swing trading:
 * SMA, EMA, and RSI of the close price. All methods are defensive and will
 * return {@link OptionalDouble#empty()} when there is not enough data or
 * invalid parameters are provided, instead of throwing.</p>
 */
public final class Indicators {
   ...
   *
   * <p>The SMA is calculated over the last {@code period} candles in the list.
   * The list is assumed to be ordered from oldest (index 0) to newest (last index).</p>
   *
   * @param candles ordered list of candles (oldest first)
   * @param period  number of candles used in the average; must be {@code > 0}
   * @return the SMA value, or {@link OptionalDouble#empty()} if
   *         {@code candles == null}, {@code period <= 0}, or there are fewer
   *         than {@code period} candles
   */
   ...
   *
   * <p>The EMA is initialized from the close price of the first candle in the
   * lookback window and then updated forward using the standard smoothing factor
   * {@code k = 2 / (period + 1)}.</p>
   *
   * @param candles ordered list of candles (oldest first)
   * @param period  number of candles in the EMA window; must be {@code > 0}
   * @return the EMA value, or {@link OptionalDouble#empty()} if
   *         {@code candles == null}, {@code period <= 0}, or there are fewer
   *         than {@code period} candles
   */
   ...
   *
   * <p>The implementation uses the classic average gain / average loss formula.
   * It looks at {@code period} price changes (so it needs at least
   * {@code period + 1} candles), computes average up-moves and down-moves, and
   * returns a value between 0 and 100 where high values (e.g. &gt; 70) indicate
   * strong recent upside momentum.</p>
   *
   * @param candles ordered list of candles (oldest first)
   * @param period  number of changes used to compute the RSI; must be {@code > 0}
   * @return the RSI value (0–100), or {@link OptionalDouble#empty()} if
   *         {@code candles == null}, {@code period <= 0}, or there are fewer
   *         than {@code period + 1} candles
   */
  public static OptionalDouble rsiClose(List<Candle> candles, int period) {
    if (candles == null || candles.size() < period + 1 || period <= 0) return OptionalDouble.empty();
