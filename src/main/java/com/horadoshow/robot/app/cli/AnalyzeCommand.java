...
import com.horadoshow.robot.analysis.Indicators;
import com.horadoshow.robot.analysis.SupportResistance;
import com.horadoshow.robot.analysis.SupportResistance.Level;
...
@Command(name = "analyze", description = "Fetch candles and print a quick swing-trade analysis (indicators + S/R).")
public final class AnalyzeCommand implements Runnable {
  private static final ZoneId SAO_PAULO = ZoneId.of("America/Sao_Paulo");
  private static final DateTimeFormatter TS = DateTimeFormatter.ofPattern("yyyy-MM-dd").withZone(SAO_PAULO);
  ...
  public void run() {
    ...
    System.out.println("Last candle: " + TS.format(last.time())
        + " O=" + last.open()
        + " H=" + last.high()
        + " L=" + last.low()
        + " C=" + last.close()
  ...
  private static void printTop(List<Level> levels, int max) {
    ...
    for (int i = 0; i < Math.min(max, levels.size()); i++) {
      ...
      System.out.println("  " + l.price() + " (touches=" + l.touches() + ", last=" + TS.format(l.lastTouch()) + ")");
    }
  }
}
