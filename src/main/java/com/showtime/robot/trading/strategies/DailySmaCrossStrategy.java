package com.showtime.robot.trading.strategies;
...
/**
 * Simple example strategy that trades based on a daily close short/long simple moving average
 * crossover.
 *
 * <p>When the short SMA is above the long SMA and there is no open position, it buys a fixed
 * fraction of cash. When the short SMA drops below the long SMA and there is an open position, it
 * exits the position.
 */
public final class DailySmaCrossStrategy implements Strategy {
   ...
   * @param longPeriod number of bars in the long moving average window (must be greater than {@code
   *     shortPeriod})
   */
