 ...
 *
 * <p>The algorithm first finds swing highs/lows (pivots) in the candle series and then clusters
 * them into price bands within a given tolerance. This is intended for swing-trade style analysis
 * and provides approximate zones, not exact tick levels.
 */
public final class SupportResistance {
   ...
   *
   * <ol>
   *   <li>Scan for local pivot highs and lows using {@code pivotLookback} candles on each side.
   *   <li>Cluster nearby pivots into horizontal bands if their relative price difference is within
   *       {@code tolerancePct} (e.g. 0.005 for 0.5%).
   *   <li>Return supports and resistances sorted by significance (touch count, recency, price).
   * </ol>
   *
   * @param candles ordered list of candles (oldest first); must not be {@code null}
   * @param pivotLookback number of candles to look back/forward when deciding if a point is a local
   *     high/low; must be {@code >= 1}
   * @param tolerancePct maximum relative distance between prices to be merged into the same level
   *     (e.g. 0.005 = 0.5%); must be {@code > 0}
   * @return a {@link Levels} object; if there are not enough candles to form pivots the lists will
   *     be empty
   ...
   *
   * <p>Order: more touches first, then most recent touch, then lower price.
   */
  private static Comparator<Level> levelSort() {
    return Comparator.comparingInt(Level::touches)
        .reversed()
    ...
    for (Pivot p : pivots) {
      ...
      if (best == null) {
        ...
        best.price =
            normalizePrice(
                BigDecimal.valueOf(
                    (best.price.doubleValue() * 0.7) + (p.price.doubleValue() * 0.3)));
      }
    }

    return clusters.stream().map(c -> new Level(c.type, c.price, c.touches, c.lastTouch)).toList();
  }

  /**
   * Normalizes price to 2 decimal places using {@link RoundingMode#HALF_UP}, to avoid noisy
   * fractions in reported levels.
   */
