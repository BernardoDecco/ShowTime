package com.showtime.robot.broker.paper;
  ...
  public OrderResponse cancelOrder(String orderId) {
    if (orderId == null || orderId.isBlank()) {
      return new OrderResponse(
          "", OrderStatus.REJECTED, Instant.now(), java.util.Optional.of("orderId is required"));
    }
    OrderRequest removed = orders.remove(orderId);
    if (removed == null) {
      return new OrderResponse(
          orderId, OrderStatus.REJECTED, Instant.now(), java.util.Optional.of("order not found"));
    }
